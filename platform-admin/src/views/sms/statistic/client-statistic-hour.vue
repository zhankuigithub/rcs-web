...
<script>
import { hoursPage } from "@/api/sms/clientStatistic";
import {
    getClientList
...
export default {
  ...
  methods: {
    fetchList() {
      ...
      hoursPage(this.queryParams).then((res) => {
    ...
    gotoDetail(index,queryType){
        this.$router.push({
            path: "/sms/statistic/client-statistic-hour-detail",
            query: {
                hourInfo:JSON.stringify(this.dataList[index]),
                queryType:queryType,
                beginDt: this.dateFormate(this.dateRange[0]),
                endDt: this.dateFormate(this.dateRange[1]),
            },
        });
    }
  },
