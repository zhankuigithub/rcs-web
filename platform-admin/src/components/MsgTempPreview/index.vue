<template>
  <div class="simulator-wrap">
    ...
    <div class="roll-cont">
      ...
      <div v-if="(messageType + '' == '2' || messageType + '' == '3') && cards.length > 0  "
      ...
      >
        ...
        <ul id="cardDomObj" class="card-wrap-ul">
          <li class="card-wrap-ul-li" v-for="(cardInfo,index) in cards" :key="index"
              :class="index == 0?'multi-card-item-first':''"
          >
            <div
              v-if="JSON.stringify(cardInfo.material) != '{}' || cardInfo.suggestions.length > 0 || cardInfo.title.length > 0 || cardInfo.description.length > 0"
              class="card-wrap" :class="cards.length == 1 ? 'single-card':'muti-card'"
            >
              <div
                :class="orientation + '' == '2'?  (imageAlignment + '' == '1'?  'card-horizontal':'card-horizontal-right'):'card-vertical'"
              >
                <div class="card-head">
                  <div class="media-wrap" :class="cardInfo.height == 3?'big':cardInfo.height == 1?'small':''"
                       v-if="cardInfo.material"
                       :style="cardInfo.material.type == 2 ? 'background-image: url(' +  cardInfo.material.thumbnailUrl + ');' : ''"
                  >
                    <img v-if="cardInfo.material.type + '' == '1'" class="grid-item-img"
                         :src="cardInfo.material.thumbnailUrl !== null ? cardInfo.material.thumbnailUrl:(cardInfo.material.fileUrl != undefined && cardInfo.material.fileUrl.length > 0) ? cardInfo.material.fileUrl:cardInfo.material.sourceUrl"
                    >
                    <video v-if="cardInfo.material.type + '' == '3'" class="grid-item-img"
                           :src="(cardInfo.material.fileUrl != undefined && cardInfo.material.fileUrl.length > 0) ? cardInfo.material.fileUrl:cardInfo.material.sourceUrl"
                           controls="controls" :poster="cardInfo.material.thumbnailUrl"
                    ></video>
                    <audio v-if="cardInfo.material.type + '' == '2'" class="grid-item-img audio"
                           :src="(cardInfo.material.fileUrl != undefined && cardInfo.material.fileUrl.length > 0) ? cardInfo.material.fileUrl:cardInfo.material.sourceUrl"
                           controls="controls"
                    ></audio>
                  </div>
                ...
                <div class="card-cont-wrap">
                  ...
                  <div class="card-buttons-wrap" style="width: 100%;">
                    <div class="card-btn align-items-center justify-content-center"
                         v-for="(buttonItem,index) in cardInfo.suggestions" :key="index"
                         @click="onCardButtonClick(buttonItem,index)"
                    >
      ...
      <div v-if="messageType + '' == '4' && position !== null && position.length != '{}'"
           class="msg-warp receivemsg-warp"
      >
        <img v-if="appInfo != null" class="chat-head-avatar" :src="appInfo.logoUrl"/>
      ...
      <div v-if="backType !== 0">
        <span></span><span style="font-size: 12px;font-weight: bold;margin-left: 15%;color: red"
      >{{ messageBackTypeFormat() }}</span>
        <br/>
    ...
    <div class="bottom-warp footer">
      <div id="floatingButton" class="float-menu-content flex-row">
        <div v-for="(item,index) in floatingButtons" :key="index" class="float-menu-item"
             :class="selectFloatBtnIndex + '' == index + '' ? 'active':''" @click="onFloatButtonClick(index)"
        >
          {{ item }}
        </div>
      </div>
...
import { dictMessageBackType } from '@/assets/initdata'
...
export default {
  ...
  methods: {
    ...
    messageBackTypeFormat() {
      return this.selectDictLabel(this.messageBackTypeList, this.backType + '')
    }
  }
}
</script>
