...
@Service
public class CuUploadStrategy extends AbstractUploadStrategy {
    ...
    public void upload(Material material, int uploadMode) throws IOException {
        ...
        Chatbot chatbot = chatbotMapper.selectOne(new LambdaQueryWrapper<Chatbot>()
                ...
                .eq(Chatbot::getCarrierId, Carrier.CUCC)
                .eq(Chatbot::getStatus, 1)
                .eq(Chatbot::getAuditStatus, 1)
                .select(Chatbot::getChatBotId));

        if (chatbot == null) {
            throw new ServerException("联通失败：此应用下没有正在启用的chatbot");
        }
        ...
        if (count > 0) {
            throw new ServerException("联通失败：此素材已经提交过审核了");
        }

        String sourceUrl = UrlUtil.buildMinIoURL(material.getSourceUrl());

        JsonNode jsonNode = rcsService.cuUploadChatBotMaterial(chatbot.getChatBotId(), sourceUrl, UploadModeEnum.getModeByType(uploadMode));

        if (jsonNode != null) {

            if (jsonNode.get("errorCode").asInt() != 0) {
                throw new ServerException("联通失败：" + jsonNode.get("errorMessage").asText());
            }

            if (jsonNode.has("fileInfo") && jsonNode.get("fileInfo") != null && !jsonNode.get("fileInfo").asText().equals("null")) {
            ...
            if (jsonNode.has("fileIdList") && jsonNode.get("fileIdList") != null && !jsonNode.get("fileIdList").asText().equals("null")) {
                JsonNode fileIdList = jsonNode.get("fileIdList");
                fileIdList.forEach(node -> materialAuditRecordMapper.saveMaterialAuditRecord(node.asText(), material.getId(), Carrier.CUCC, chatbot.getChatBotId(), uploadMode, "", "", "", null, 0, 0));
            }
