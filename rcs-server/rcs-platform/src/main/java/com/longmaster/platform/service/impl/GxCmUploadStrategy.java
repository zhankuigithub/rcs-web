...
@Service
public class GxCmUploadStrategy extends AbstractUploadStrategy {
    @Resource
    ...
    public Long getCarrierId() {
        return Carrier.GUANG_XI_CMCC;
    }

    @Override
    public void upload(Material material, int uploadMode) throws IOException {
        Long appId = material.getAppId();  // 获取移动下的该应用下的chatbot
        Chatbot chatbot = chatbotMapper.selectOne(new LambdaQueryWrapper<Chatbot>().eq(Chatbot::getAppId, appId).eq(Chatbot::getCarrierId, Carrier.GUANG_XI_CMCC).select(Chatbot::getChatBotId));

        if (chatbot == null) {
            throw new ServerException("广西移动失败：此应用下未创建chatbot");
        }
        ...
        if (count > 0) {
            throw new ServerException("广西移动失败：此素材已经提交过审核了");
        }

        String tid = rcsService.gxCmUploadChatBotMaterial(chatbot.getChatBotId(), UrlUtil.buildMinIoURL(material.getSourceUrl()));

        if (StrUtil.isEmpty(tid)) {
            throw new ServerException("广西移动失败：tid为空");
        }

        materialAuditRecordMapper.saveMaterialAuditRecord(tid,
                material.getId(), Carrier.GUANG_XI_CMCC,
